...
TEST_CASE( "Inductive sets are correct", "[compute_ind]" ) {
    ...
    t.add_transition_by_levels(t_init, {'0', '1'}, t_init);
    t.add_transition_by_levels(t_init, {'1', '0'}, t_init);
    ...
    Nfa ind {compute_ind(daf, t, std::make_shared<mata::EnumAlphabet>(concrete_alphabet), std::make_shared<mata::EnumAlphabet>(abstract_alphabet), true, logging::VerbosityLevel::DEBUGV)};
