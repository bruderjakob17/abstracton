...
int main() {
    for (int i = 0; i < 5; ++i) {
        ...
        TICK();
        bool is_universal0 = mata::ext::is_universal_antichains(aut, std::make_shared<mata::AlphabetLevels>(alphabets), &cex0);
        TOCK();
        std::cout << "Result: " << is_universal0 << std::endl;

        std::cout << "Using inclusion:" << std::endl;
        TICK();
        bool is_universal1 = mata::ext::is_universal_antichains_by_inclusion(aut, std::make_shared<mata::AlphabetLevels>(alphabets), &cex1);
        TOCK();
        std::cout << "Result: " << is_universal1 << std::endl;

        std::cout << "Lazy (without subsumption):" << std::endl;
        TICK();
        bool is_universal2 = mata::ext::is_universal_lazy(aut, std::make_shared<mata::AlphabetLevels>(alphabets), &cex2);
        TOCK();
